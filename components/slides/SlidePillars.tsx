...
export default function SlidePillars() {
    return (
        <SlideWrapper className="bg-slate-900 text-white relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5 w-full relative z-10 px-4">
                {pillars.map((pilar, index) => (
                    <motion.div
                        key={index}
                        className="flex flex-col items-center text-center group cursor-pointer bg-slate-800/30 backdrop-blur-sm p-4 md:p-5 rounded-2xl border border-slate-700/50 hover:border-proteo-orange/50 transition-all h-full"
                        initial={{ opacity: 0, y: 50 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ delay: index * 0.2, duration: 0.5 }}
                        whileHover={{ y: -10 }}
                    >
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-2 border-slate-700 flex items-center justify-center mb-3 md:mb-4 group-hover:border-proteo-orange group-hover:bg-proteo-orange/10 transition-all duration-500 relative overflow-hidden shrink-0">
                            ...
                            <pilar.icon className="w-8 h-8 md:w-10 md:h-10 text-slate-300 group-hover:text-proteo-orange transition-colors duration-300 relative z-10" />
                        </div>

                        <h3 className="text-lg md:text-xl font-bold mb-1 md:mb-2 group-hover:text-proteo-orange transition-colors">{pilar.title}</h3>
                        <h4 className="text-xs md:text-sm text-cyan-300 mb-2 md:mb-3 font-medium">{pilar.subtitle}</h4>
                        <p className="text-slate-200 text-[10px] md:text-xs mb-3 md:mb-4 leading-relaxed">{pilar.description}</p>

                        {/* Metrics */}
                        <div className="mt-auto w-full space-y-1.5 md:space-y-2 pt-3 md:pt-4 border-t border-slate-700/50">
