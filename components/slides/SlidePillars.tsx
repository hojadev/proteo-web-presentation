        ...
        <SlideWrapper className="bg-slate-900 text-white relative">
            ...
            <div className="flex flex-col md:flex-row justify-center items-stretch gap-8 w-full relative z-10">
                {pillars.map((pilar, index) => (
                    <motion.div
                        key={index}
                        className="flex-1 flex flex-col items-center text-center group cursor-pointer bg-slate-800/30 backdrop-blur-sm p-8 rounded-2xl border border-slate-700/50 hover:border-proteo-orange/50 transition-all"
                        initial={{ opacity: 0, y: 50 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ delay: index * 0.2, duration: 0.5 }}
                        whileHover={{ y: -10 }}
                    >
                        <div className="w-32 h-32 rounded-full border-2 border-slate-700 flex items-center justify-center mb-6 group-hover:border-proteo-orange group-hover:bg-proteo-orange/10 transition-all duration-500 relative overflow-hidden">
                            ...
                            <pilar.icon className="w-14 h-14 text-slate-300 group-hover:text-proteo-orange transition-colors duration-300 relative z-10" />
                        </div>

                        <h3 className="text-2xl font-bold mb-2 group-hover:text-proteo-orange transition-colors">{pilar.title}</h3>
                        <h4 className="text-base text-blue-300 mb-4 font-medium">{pilar.subtitle}</h4>
                        <p className="text-slate-400 text-sm mb-6 leading-relaxed">{pilar.description}</p>

                        {/* Metrics */}
                        <div className="mt-auto w-full space-y-3 pt-6 border-t border-slate-700/50">
